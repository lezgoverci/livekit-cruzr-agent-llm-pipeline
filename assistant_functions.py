...
class AssistantFnc(llm.FunctionContext):
    ...
    async def add_google_sheets_row(
        self,
        name: Annotated[str, llm.TypeInfo(description="Name of the contact person")],
        email: Annotated[str, llm.TypeInfo(description="Email address of the contact person")]
    ):
        """
        Adds a new row to a Google Sheet using n8n webhook with name and email.
        """
        webhook_url = "https://n8n.xponent.ph/webhook/livekit-cruzr-sheets"
    ...
    async def get_email_by_name(
        self,
        contact_name: Annotated[str, llm.TypeInfo(description="Name of the contact person")]
    ):
        """
        Retrieves the email address of a contact person from a Google Sheet using their name.
        """
        webhook_url = "https://n8n.xponent.ph/webhook/livekit-cruzr-get-email"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(webhook_url, json=payload) as response:
                if response.status == 200:
                    ...
                    return data.get("email", "Email not found.")
                else:
                    raise Exception(f"Failed to retrieve email. Status code: {response.status}")
